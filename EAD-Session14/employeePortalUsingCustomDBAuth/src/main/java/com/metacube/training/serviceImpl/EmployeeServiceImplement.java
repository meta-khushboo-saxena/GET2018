...
public class EmployeeServiceImplement implements EmployeeServiceInterface {
	...
	@Autowired
	private SkillsRepository<SkillsMaster> skillRepository;
	...
	public List<Employee> getAllInfo() {
		return empRepo.findByEnabled(true);
	}
	...
	public List<Employee> searchBy(String filter, String attributeName) {
		if (filter.equals("name")) {
			return empRepo.findByFirstNameContainingIgnoreCase(attributeName);
		} else if (filter.equals("skills")) {
			return empRepo.findBySkills(skillRepository.findBySkillName(attributeName));
		} else if (filter.equals("project")) {
			return empRepo.findByProjects(projectRepository.findByName(attributeName));
