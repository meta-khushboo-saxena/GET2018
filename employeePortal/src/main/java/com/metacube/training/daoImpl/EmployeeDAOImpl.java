...
public class EmployeeDAOImpl implements EmployeeDAO {
	...
	private final String SQL_INSERT_TOKEN = "UPDATE employee SET token = ? WHERE email = ? ";

	private final String SQL_RESET_PASSWORD = "UPDATE employee SET password = ? WHERE email = ? ";

	private final String SQL_DELETE_TOKEN = "UPDATE employee SET token = NULL WHERE email = ? ";
	...
	public Boolean insertToken(ForgotPasswordToken forgotPasswordToken) {
		return jdbcTemplate.update(SQL_INSERT_TOKEN,
				forgotPasswordToken.getToken(), forgotPasswordToken.getEmail()) > 0;
	}

	@Override
	public boolean deleteToken(String email) {
		return jdbcTemplate.update(SQL_DELETE_TOKEN, email) > 0;
	}

	@Override
	public Boolean reset(String email, String password) {
		return jdbcTemplate.update(SQL_RESET_PASSWORD, password, email) > 0;
	}
}
